...
/* 28 */       if (p.hasPermission(Config.getSetting("DevPerm"))) {
/* 29 */         ICloudPlayer cloudPlayer = getCloudPlayer(p);
/* 30 */         ICloudServerGroup group = CloudAPI.getInstance().getCloudServiceGroupManager().getServerGroupByName(Config.getSetting("TestserverName"));
...
/*    */         
/* 38 */         if (server == null) {
/* 39 */           sender.sendMessage(TextComponent.fromLegacyText(String.valueOf(Proxy.PREFIX) + Config.getSetting("ServerStarting").replace("<ServerName>", Config.getSetting("TestserverName"))));
/* 40 */           group.startNewService();
/* 41 */         } else if (server.getState().equals(ServiceState.CLOSED)) {
/* 42 */           sender.sendMessage(TextComponent.fromLegacyText(String.valueOf(Proxy.PREFIX) + Config.getSetting("ServerStarting").replace("<ServerName>", Config.getSetting("TestserverName"))));
/* 43 */           server.start();
/*    */         } else {
/* 45 */           sender.sendMessage(TextComponent.fromLegacyText(String.valueOf(Proxy.PREFIX) + Config.getSetting("TestserverSuccess")));
...
/*    */       } 
/*    */     } else {
/* 52 */       sender.sendMessage(TextComponent.fromLegacyText(String.valueOf(Proxy.PREFIX) + Config.getSetting("CommandOnlyIngame")));
/*    */     } 
/*    */   }
/*    */   
...
/* Location:              C:\Users\Leon\Downloads\LbGameBungeeCordSystem(1).jar!\de\tellem\commands\TestserverCommand.class
